...
补齐逻辑（`lib/client.js`）：
1. 找不到锚点行时，优先**点击该“加载更早”按钮**（先用 CSS-modules 类名后缀 `_older`；文案兜底刻意收紧为 `older/earlier/更早/加载(更早|更多|历史)` 并排除 `loading/加载中` —— 裸 `加载`/`历史` 会误点“历史记录/加载更多”等无关控件）、否则向上走一屏；**点击有节流**（≥500ms 一次，连续 3 次没加载出新内容就停手）；
