...
2. **Command Line Tools 没有 XCTest**：`swift test` 会报 `error: XCTest not available`（且 `xcrun --show-sdk-platform-path` 失败）。Core 的测试套件需完整 Xcode 或 CI（macOS runner 自带 Xcode；仓库 `.github/workflows/tests.yml` 的 `swift tests (XCTest)` job 负责全绿证据）。CLT 本机只能 `swift build` 验证编译。
