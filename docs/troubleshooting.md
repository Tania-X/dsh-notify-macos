...
## 25. 右划清除的动效取值，以及一个「决定不做」

### 25.1 动效：位移占满全程、淡出挪到尾部

**用户反馈**：往右拖拽清卡时「淡出效果要更明显，往右弹出并淡化的感觉」。原实现位移与淡出**同时**进行、共用 0.22s，而拖拽本身已经把卡片带到屏幕边缘 —— 剩下的位移极短，观感上只剩「一下子没了」。

现在（`Sources/dshNotifyServer/main.swift` 的 `Dismiss` 常量）：

| 常量 | 值 | 作用 |
| --- | --- | --- |
| `travelSeconds` | 0.34 | 位移全长（`.easeIn`，加速甩出） |
| `fadeDelayFraction` | 0.55 | 位移进行到 55% 才开始淡出 —— 先看见"飞出去"，再"化掉" |
| `fadeSeconds` | 0.15 | 淡出时长 |
| `minTravel` | 220 | 最小位移兜底：拖到屏幕边缘也保证还有 220px 可见飞行 |
| `offscreenMargin` | 24 | 终点 = 屏幕右缘 + 卡片宽度 + 24（整卡彻底出屏） |

**连带修掉一个竞态（评审判 [2]，且是拉长动画放大的）**：dismiss 动画期间卡片仍留在栈里（要等动画结束才 `removeNow`），这 0.34s 内若同会话又来一条完成通知，合并逻辑会把它并进**正在飞出的卡** —— 通知画在即将消失的窗口上，用户看不到。现在合并时跳过 `isDismissing` 的卡片，新通知拿到自己的新卡；冒烟里有一条能稳定复现该竞态的断言（造出"正在飞出"状态 → 立刻推同会话完成 → 断言栈里多了一张新卡）。

### 25.2 决定不做：success/fail 卡片「触底自动清空」
