...
## 30. 跳转完成后还在和用户抢滚动条（实机反馈）

**用户反馈**：点卡片确实跳过去了，但随后**一段时间内**手动上下滚动会被一次次拽回跳转位置 —— "如果已经定位到了该跳转的位置，这个可以提前结束"。

**根因**（`lib/client.js`）：跳转要对付两件事 —— 长会话**分页加载历史**、以及会话切换后的渲染延迟 —— 所以实现成了"**连续几轮校正滚动位置**"。问题是校正的收尾条件写得太保守，而且完全没有把"用户自己动了"当成信号：

| 位置 | 旧行为 | 后果 |
| --- | --- | --- |
| `pinToNewest` | 每 120ms 无条件 `scrollTop = scrollHeight`，要连续 **3 轮**高度不变才停，否则一直校正到 **6 秒**超时 | 内容还在分页时（或用户在两次 tick 之间向上滚一点）就被反复拽回底部 |
| `scrollToTurn` | 找到行后**每 tick 重新对齐**（注释原文 "Re-align every tick"），上限 **8 秒** | 同上；用户手动滚动被当成"需要重新对齐" |
| 两者 | 不监听任何用户输入 | 分不清"还没到位"和"用户不想你管了" |

**为什么特别容易在 0.1.5-rc.2 上撞见**：新版前端移除了位置锚点（§29），于是走的永远是 `pinToNewest` 那条 6 秒路径。

**修法**（两个原则，都很便宜）：

1. **用户一接管就立刻停手**：只监听**真正的用户输入** —— `wheel` / `touchstart` / `touchmove` / `mousedown` / 方向键类 `keydown`（普通字符键不算，否则打字也会打断）；一发生就移除监听并放弃剩余校正。关键技巧：我们自己的 `scrollTop = …` **不会**触发这些事件，所以不需要去猜"这次 scroll 事件是程序还是用户引起的"（那才是容易写错的地方）。
2. **到位就收工**：`pinToNewest` 改成"到底 + 内容不再增长"即结束，并且**只在没到底时才写 `scrollTop`**；`scrollToTurn` 的对齐判定从三轮收成两轮。
...
- `agent/status` 回到 idle 时 → 若本轮失败过就**不发完成卡**，并清掉记录（下次仍能正常弹）；
- `turn/start` → 清掉记录：**新一轮开始，旧的失败结局必须作废**，否则一个失败过的会话之后再也弹不出完成卡。
