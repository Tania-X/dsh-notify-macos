...
- **修法**：记住"本轮是否以失败收场"，agent 回到 idle 时若本轮失败过就**不发完成卡**（并发完即清）。
  新的一轮从 `turn/start` 开始，那时旧结局作废 —— 否则失败过的会话之后再也弹不出完成卡；
...
手动上下滚动会被一次次拽回跳转位置（在 0.1.5-rc.2 这类新版前端上尤其明显，因为位置锚点
被移除后走的永远是"钉到最新"那条 6 秒的路径）。

- **用户一接管就立刻停手**：跳转后的滚动校正期间监听**真正的用户输入**（`wheel` / 触摸 /
  方向键 / `mousedown`），一旦发生就放弃剩余校正。我们自己的 `scrollTop = …` 赋值不会
  触发这些事件，所以不需要去猜"这次 scroll 是谁引起的"；
- **到位就收工**：`pinToNewest` 改成"到底 + 内容不再增长"即结束（原来要连续三轮、
  失败则一直校正到超时），且**只在没到底时才写 `scrollTop`**；`scrollToTurn` 的对齐
  判定从三轮收成两轮；
- 新增 `test/client-scroll.test.js`（7 条）：用假 DOM 装出"历史还在分页加载 + 用户中途滚动"
  的真实场景。这块以前**完全没有测试**；实测咬合：把"接管就停手"短路掉，同一场景会被
  强行写 51 次 `scrollTop`（正是用户描述的手感），断言随即变红。

细节见 `docs/troubleshooting.md` §30。
