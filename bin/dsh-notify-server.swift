...
final class CardView: NSView {
    ...
    /// The SVG path uses only absolute M / C / Z commands with a 50x50
    /// viewBox; tokens are extracted with a regex and mapped onto the target
    /// rect (the source is already normalized to the box the favicon ships in).
    static func whaleMarkPath(in rect: NSRect) -> NSBezierPath {
        let path = NSBezierPath()
        let scale = rect.width / 50.0
        // Split into (command letter, following numbers) groups.
        ...
        func moveTo(_ x: CGFloat, _ y: CGFloat) {
            let p = NSPoint(x: rect.minX + x * scale, y: rect.maxY - y * scale)
            path.move(to: p)
        }
        func curveTo(_ c1x: CGFloat, _ c1y: CGFloat, _ c2x: CGFloat, _ c2y: CGFloat, _ x: CGFloat, _ y: CGFloat) {
            let c1 = NSPoint(x: rect.minX + c1x * scale, y: rect.maxY - c1y * scale)
            let c2 = NSPoint(x: rect.minX + c2x * scale, y: rect.maxY - c2y * scale)
            let p = NSPoint(x: rect.minX + x * scale, y: rect.maxY - y * scale)
        ...
        for match in matches {
            ...
            let cmd = ns.substring(with: cmdRange)
            ...
            case "Z":
                path.close()
                // (absolute-path close; no relative continuation follows)
            default:
                break
            }
        }
