...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** Keys that mean "the user is scrolling" (used to hand over immediately). */
     ...
     * 用户接管就立刻停手。
     *
     * 跳转要在"页面还在分页加载历史"的前提下把视口带到目标位置，所以必须连续几轮
     * 校正滚动位置。但**用户一旦自己滚动或按键，含义就不是"还没到位"，而是"你别管了"**
     * ——再校正就是和用户抢滚动条（实测症状：跳转完成后 6~8 秒内手动上下滚，会被一次次
     * 拽回跳转位置）。
     *
     * 只监听**真正的用户输入**（wheel / 触摸 / 方向键 / 鼠标按下）；我们自己的
     * `scrollTop = …` 赋值不会触发这些事件，所以不需要去猜"这次 scroll 事件是谁引起的"。
     */
    function watchUserTakeover() {
      let takenOver = false;
      ...
      const stop = () => {
        ...
        for (const type of events) window.removeEventListener(type, onUserInput, true);
      };
      ...
      return { takenOver: () => takenOver, stop };
    }
    ...
    function loadOlderButton(scroller) {
      ...
      for (const button of scroller.querySelectorAll("button")) {
        ...
        // Word boundaries keep "Folder"/"Older versions" and "加载更多" out.
        if (/\b(older|earlier)\b|更早|加载更早/i.test(label)) return button;
      }
      return null;
    }
    ...
    function scrollToTurn(turn, timeoutMs, onMiss) {
      ...
      const tick = () => {
        if (guard.takenOver()) return;   // 用户接管：立刻放弃剩余的校正
        ...
        if (scroller !== null) {
          ...
          if (row !== null) {
            ...
            // 对齐且内容不再增长 → 到位收工（两轮足够，早于三轮就是早几百毫秒还回控制权）
            if (stableRounds >= 2 && Math.abs(delta - target) <= 4) {
              guard.stop();
              return;
            }
          } else {
            ...
            if (now - lastClickAt >= 500) {
              ...
              if (older !== null) {
                // Judge the PREVIOUS click only when we are about to click again:
                // windows where no click happened (button missing or disabled)
                // must not count as "loaded nothing", or a temporarily busy
                // control would stop the seek early.
                const grew = heightAfterClick < 0 || scroller.scrollHeight > heightAfterClick;
                clicksWithoutGrowth = grew ? 0 : clicksWithoutGrowth + 1;
                if (clicksWithoutGrowth < 3) {
                  lastClickAt = now;
        ...
        if (Date.now() < deadline) {
          ...
          guard.stop();
          if (!found && typeof onMiss === "function") onMiss();
        }
      };
      tick();
    }
    ...
    function pinToNewest(timeoutMs) {
      ...
      const tick = () => {
        ...
        const scroller = document.querySelector("[data-conversation-scroll]");
        if (scroller !== null) {
          // 只在**没到底**时才写 scrollTop：已经到底就别再动它 —— 否则用户在两次
          // tick 之间往上滚一点，下一 tick 就把他拽回去（这就是"锁定"的手感来源之一）。
          const atBottom =
          ...
          lastHeight = scroller.scrollHeight;
          // 到底 + 内容不再增长 → 到位收工（旧代码要连续三轮，且在长会话分页期间会
          // 一直校正到 6 秒超时）
          if (atBottom && stableRounds >= 1) {
            guard.stop();
            return;
          }
        }
     ...
     */
    function apply(ctx) {
      // 防御式加载：前端契约（ctx.get / sessions.open / window）变化时，client 半区
      // 只应"安静失效"，绝不抛错影响页面本身。
      try {
        if (typeof ctx?.get !== "function") {
          console.warn("[dsh-notify-macos] ctx.get 不可用（前端契约变化？）；跳转功能已禁用");
          return;
        }
        ...
        const onChange = () => {
          try {
            ...
            console.warn("[dsh-notify-macos] 处理跳转 hash 失败（已忽略）：", error?.message ?? error);
          }
        };
        ...
        console.warn("[dsh-notify-macos] client 半区加载失败（已降级）：", error?.message ?? error);
      }
    }
