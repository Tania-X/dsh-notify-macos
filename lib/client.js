...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function loadOlderButton(scroller) {
      ...
      const byClass = scroller.querySelector('[class*="_older"] button');
      ...
      const tick = () => {
        ...
        if (scroller !== null) {
          ...
          if (row !== null) {
            ...
            const now = Date.now();
            const mayClick = older !== null && clicksWithoutGrowth < 3 && now - lastClickAt >= 500;
            if (mayClick) {
              if (heightAfterClick > 0 && scroller.scrollHeight === heightAfterClick) {
                clicksWithoutGrowth += 1;   // that click loaded nothing new
              }
              lastClickAt = now;
