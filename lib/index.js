...
/** Ask the daemon to drop the row waiting on `ref` (best effort, no spawn). */
async function clearBlocked(cfg, sessionId, ref) {
  if (!sessionId || !ref) return;
  const socketPath = cfg.socketPath ?? defaultSocketPath();
  const ok = await sendToDaemon(socketPath, { cmd: "clear", sessionId, ref });
  if (!ok) {
    // A daemon that is not running holds no cards, so there is nothing to
    // clear and spawning one just to say "clear" would be pointless.
    return;
  }
}
...
export function apply(ctx, config = {}) {
  ...
  ctx.on("session/event", (session, event) => {
    ...
    if (shouldClearResolved(blockedState.resolved, blockedState.wasPending, event)) {
      void clearBlocked(config, sessionIdOf({ session }), blockedState.resolved);
    }
    switch (event.type) {
      case "turn/end": {
