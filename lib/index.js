 ...
 */
async function deliver(cfg, kind, message, detail, cwd, sessionTitle, sessionId) {
...
export function apply(ctx, config = {}) {
  ...
  /** True when the session is a top-level conversation (not a subagent). */
  const isRootSession = (session) => {
    if (!rootOnly) return true;
    // A missing session must never be treated as a root: optional chaining
    // alone would turn `undefined?.x === undefined` into true and raise a
  ...
  const notify = (session, kind, detail) => {
    if (!enabled || !isRootSession(session)) return;
    let title;
    try {
    ...
    void deliver(config, kind, copy[kind] ?? "任务已完成", detail, cwdOf({ session }), title, sessionIdOf({ session }));
  };
  ...
  ctx.on("agent/status", ({ agent, status }) => {
    ...
    if (!enabled || !isRootSession(agent.session)) return;
    const sessionTitle = sessionTitleOf(ctx, agent);
    void deliver(config, "completed", copy.completed, undefined, cwdOf(agent), sessionTitle, sessionIdOf(agent));
  });
  ...
  ctx.on("session/event", (session, event) => {
    ...
    switch (event.type) {
      case "turn/end": {
        ...
        // Guard the event contract: only inspect nested fields when reason is
        // actually an object (a schema change must never crash the handler).
        ...
        if (bad.includes(kind)) {
          // Skip turns that were intentionally blocked (those raise "blocked"
          // at approval/asked time and resolve as aborted/blocked on cancel).
          const subReason = reason.reason;
          const subKind =
            subReason !== null && typeof subReason === "object" && typeof subReason.kind === "string"
              ? subReason.kind
              : "";
          if (kind === "aborted" && subKind === "disposed") return;
          const err = reason.error;
          const detail =
            err !== null && typeof err === "object" && typeof err.message === "string"
              ? err.message
              : kind;
          notify(session, "error", detail);
        }
        break;
      }
      case "approval/asked": {
