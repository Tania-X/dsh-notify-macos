...
export const Config = z.object({
   ...
   *   - "open-web":    open the DeepSeek Harness Web UI in the default browser
 ...
 */
async function deliver(cfg, message, cwd, sessionTitle) {
  ...
  if (cfg.clickAction === "open-web") payload.url = cfg.webUrl;
...
export function apply(ctx, config = {}) {
  ...
  ctx.on("agent/status", ({ agent, status }) => {
    ...
    const body = sessionTitle
      ...
      : message;
    void deliver(config, body, cwdOf(agent), sessionTitle);
  });
