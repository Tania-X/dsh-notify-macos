...
export const Config = z.object({
  ...
  /** Notification body; the card's title line shows the session name. */
  message: z.string().default("任务完成，点击查看详情"),
...
export function apply(ctx, config = {}) {
  ...
  const message = config.message ?? "任务完成，点击查看详情";
  ...
  ctx.on("agent/status", ({ agent, status }) => {
    ...
    // The card's title line is the session name (handled by the daemon);
    // the body is the generic completion copy.
    void deliver(config, message, cwdOf(agent), sessionTitle, sessionIdOf(agent));
  });
