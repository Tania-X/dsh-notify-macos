...
/** A spawn that survives this long counts as healthy (crash-loop detector). */
const HEALTHY_DAEMON_MS = 2000;

/** When the daemon was last spawned (throttles respawn attempts). */
let daemonSpawnedAt = 0;
/** True when the last spawn died quickly (crash loop) — armed by the exit handler. */
let daemonLastSpawnFailed = false;
...
export function shouldStartDaemon({
  hasHandle, handleDead, lastSpawnedAt, now, lastSpawnFailed = false, retryAfterMs = 2000
}) {
  if (hasHandle && !handleDead) return false;   // alive → never spawn a rival
  // Dead or missing handle: replace it at once, unless the previous replacement
  // also died immediately (broken binary / unwritable socket) — then space the
  // attempts so a crash loop cannot spawn a process per notification.
  if (!lastSpawnFailed) return true;
  if (typeof lastSpawnedAt !== "number" || lastSpawnedAt <= 0) return true;
  return now - lastSpawnedAt >= retryAfterMs;
}

/** Current daemon bookkeeping for shouldStartDaemon. */
function daemonState(now = Date.now()) {
  const handle = daemonProcess;
  // `exitCode`/`signalCode` are set once the child has actually exited;
  // `killed` only says we sent a signal (the process may still be running).
  const handleDead = handle !== null && (handle.exitCode !== null || handle.signalCode !== null);
  return {
    hasHandle: handle !== null,
    handleDead,
    lastSpawnedAt: daemonSpawnedAt,
    lastSpawnFailed: daemonLastSpawnFailed,
    now
  };
}
...
function startDaemon(serverPath, socketPath) {
  try {
    ...
    // A dead daemon must not keep blocking future spawns. Keep the spawn
    // timestamp (so a crash loop is throttled) but record whether this attempt
    // died immediately: a healthy daemon that served for a while is replaced at
    // once, while a binary that cannot start is retried at most every 2s.
    child.on("exit", () => {
      if (daemonProcess === child) {
        daemonProcess = null;
        daemonLastSpawnFailed = Date.now() - daemonSpawnedAt < HEALTHY_DAEMON_MS;
      }
    });
 ...
 */
/**
 * Build the `show` payload for one notification. Pure (no sockets) so the
 * optional fields — the position anchor `turn` and the blocked-row `ref` — can
 * be unit-tested directly instead of through the socket layer.
 *
 * @param cfg - validated plugin config.
 * @param kind - outcome kind ("completed" | "error" | "blocked").
 * @param message - body copy.
 * @param detail - structured detail (error message / tool name).
 * @param cwd - session working directory (open-folder action).
 * @param sessionTitle - human title of the session.
 * @param sessionId - session id the card deep-links to.
 * @param turn - turn anchor for the click.
 * @param ref - correlation key for a row waiting on the user.
 */
export function buildShowPayload(
  cfg, kind, message, detail, cwd, sessionTitle, sessionId, turn, ref
) {
  const payload = {
    cmd: "show",
    title: cfg.title,
  ...
  // Correlation key for a row waiting on the user, so the daemon can drop
  // exactly this row once the user resolves it in the GUI.
  if (typeof ref === "string" && ref) payload.ref = ref;
...
async function deliver(cfg, kind, message, detail, cwd, sessionTitle, sessionId, turn, ref) {
...
/**
 * Correlation key of the pending user action a `blocked` card waits on.
 *
 * The daemon stores this key on the row, so when the user resolves the
 * situation in the GUI the matching row can be dropped EXACTLY (rather than
 * guessing by kind or by "the newest blocked row").
 *
 *   approval/asked            -> `approval:<id>`   (payload: {id, toolName, callId})
 *   tool/call ask_user_question -> `ask:<callId>`   (payload: {callId, name})
 *
 * @param event - a `session/event` payload.
 * @returns the key, or null when this event does not await the user.
 */
export function blockedRef(event) {
  if (event === null || typeof event !== "object") return null;
  const data = event.data ?? {};
  if (event.type === "approval/asked") {
    // No `callId` fallback ON PURPOSE: `approval/decided` only carries `id`
    // (payload {id, outcome}), so a callId-based key could never be matched by
    // the resolver — it would only create a dangling key. Without an id the row
    // simply carries no ref and keeps the old behaviour (click to dismiss).
    const id = typeof data.id === "string" && data.id ? data.id : "";
    return id ? `approval:${id}` : null;
  }
  if (event.type === "tool/call" && data.name === "ask_user_question") {
...
/** Pull the tool call id out of a tool/result payload (shape-tolerant). */
...
/**
 * Correlation key whose pending user action has ENDED — the user decided the
 * approval or answered the question, so its card row should go away.
 *
 *   approval/decided -> `approval:<id>`  (payload: {id, outcome})
 *   tool/result      -> `ask:<callId>`   (the ask tool returns the answer)
 *
 * @param event - a `session/event` payload.
 * @returns the key, or null.
 */
export function resolvedRef(event) {
  if (event === null || typeof event !== "object") return null;
  const data = event.data ?? {};
  ...
  if (event.type === "tool/result") {
    ...
    return callId ? `ask:${callId}` : null;
  }
  return null;
}

/**
 * Whether a resolved key should be sent to the daemon.
 *
 * `wasPending` is the normal case (we raised that amber row ourselves). An
 * `approval/decided` is also acted on when we did NOT track it — that event is
 * rare and precise, so it doubles as the self-healing path after a host restart
 * lost its pending set (otherwise a stale amber card would linger forever).
 * Plain tool/result events are only acted on when we raised the row, because
 * EVERY tool call produces one.
 *
 * @param ref - key from resolvedRef(); null means nothing to do.
 * @param wasPending - whether this host instance raised that row.
 * @param event - the originating event.
 */
export function shouldClearResolved(ref, wasPending, event) {
  if (!ref) return false;
  if (wasPending) return true;
  return event?.type === "approval/decided";
}

/**
 * Per-session tracking of pending user actions, as a pure transition.
 *
 * @param state - previous state ({ pending: string[] }) or undefined.
 * @param event - a `session/event` payload.
 * @returns {{ pending: Set<string>, raised: string|null, resolved: string|null,
 *            wasPending: boolean }}
 */
export function nextPendingRefs(state, event) {
  const pending = new Set(Array.isArray(state?.pending) ? state.pending : []);
  const raised = blockedRef(event);
  if (raised) pending.add(raised);
  const resolved = resolvedRef(event);
  const wasPending = resolved !== null && pending.has(resolved);
  if (resolved) pending.delete(resolved);
  return { pending, raised, resolved, wasPending };
}

/**
 * Send one request and wait for the daemon's JSON reply.
 *
 * `sendToDaemon` resolves on WRITE, which is all a fire-and-forget `show`
 * needs. `clear` needs the answer: the daemon distinguishes "removed the row"
 * from "no such card/row" (`removed`/`reason`), and reporting that is the
 * difference between a silent no-op and a diagnosable one.
 *
 * @returns {{ ok: boolean, reply: object|null }}
 */
function requestDaemon(socketPath, payload, timeoutMs = 1500) {
  return new Promise((resolve) => {
    const socket = net.createConnection(socketPath);
    let settled = false;
    let buffer = "";
    ...
    socket.setTimeout(timeoutMs, () => finish(false, null));
    ...
    socket.on("close", () => finish(false, null));
  });
}

/**
 * Fire-and-forget with a log instead of an unhandled rejection.
 *
 * `deliver` / `clearBlocked` are async and deliberately not awaited on the event
 * path; without this, any failure inside them (socket error, serializer edge
 * case) becomes an unhandled rejection in the HOST process — i.e. the plugin
 * could take the harness down. Log and move on.
 *
 * @param promise - the pending work.
 * @param logger - plugin logger (optional).
 * @param label - what was being done, for the log line.
 */
function fireAndForget(promise, logger, label) {
  void promise.catch((error) => {
    try {
      logger?.warn?.(`[dsh-notify-macos] ${label} failed: ${error?.message ?? String(error)}`);
    } catch {
...
/**
 * Ask the daemon to drop the row waiting on `ref`.
 *
 * Recovery mirrors `deliver`: a daemon that is down (first use, killed, crashed
 * and not yet respawned) gets started and the request is retried once. Without
 * that, a card restored from the snapshot would linger on screen forever even
 * though the user already resolved the situation in the GUI — the exact case
 * this feature exists for. Whatever happens is logged, never swallowed.
 */
async function clearBlocked(cfg, logger, sessionId, ref) {
  if (!sessionId || !ref) return;
  const socketPath = cfg.socketPath ?? defaultSocketPath();
  const serverPath = cfg.serverPath ?? defaultServerPath();
  const send = () => requestDaemon(socketPath, { cmd: "clear", sessionId, ref });

  let { ok, reply } = await send();
  if (!ok && shouldStartDaemon(daemonState())) {
    startDaemon(serverPath, socketPath);
    await new Promise((resolve) => setTimeout(resolve, 400));
    ({ ok, reply } = await send());
  ...
  if (!ok) {
    logger?.warn?.(
      `[dsh-notify-macos] clear dropped (daemon unreachable) ref=${ref}; the card stays until handled by hand`
    );
    return;
  }
  if (reply && reply.removed === 0) {
    // Already handled (clicked / cleared) or no longer present: not an error.
    logger?.info?.(`[dsh-notify-macos] clear no-op (${reply.reason ?? "unknown"}) ref=${ref}`);
  }
}
...
export function apply(ctx, config = {}) {
  // 防御式加载：DSH 升级后契约可能变化，而**插件加载失败不能拖垮 harness 启动**。
  // 任何异常都只记一行日志并安静降级（卡片不弹，DSH 照常跑）。
  try {
    install(ctx, config);
  } catch (error) {
    warn(ctx, `插件加载失败，已降级为不通知：${error?.message ?? String(error)}`);
  }
}

/** Log through the plugin logger, swallowing logger failures too. */
function warn(ctx, message) {
  try {
    ctx?.logger?.warn?.(`[dsh-notify-macos] ${message}`);
  } catch {
    /* a broken logger must not turn a degradation into a crash */
  }
}

/**
 * Wrap one event handler so a malformed payload (or a bug in our own logic) is
 * logged and dropped instead of propagating into the harness event bus.
 * After an upgrade, contract drift must look like "nothing happens" — never
 * like "DSH fails to boot".
 */
function guarded(label, handler, ctx) {
  return (...args) => {
    try {
      handler(...args);
    } catch (error) {
      warn(ctx, `${label} handler 出错（已忽略）：${error?.message ?? String(error)}`);
    }
  };
}

/** The real installation: subscribes to the events it understands. */
function install(ctx, config = {}) {
  if (process.platform !== "darwin") {
    warn(ctx, "not on macOS; notifications disabled");
    return;
  }
  if (typeof ctx?.on !== "function") {
    warn(ctx, "ctx.on 不可用（DSH 契约变化？）；已降级为不通知");
    return;
  }
  ...
  /** Per-session pending user actions (see nextPendingRefs). */
  const pendings = new WeakMap();

  /**
   * 本轮（一个 agent 从 running 到 idle 之间）最后是以"失败"收场的会话。
   *
   * 为什么要记：完成卡的触发信号是 `agent/status` 的 `running → idle`，它只知道
   * "这一批 turn 排空了"，**不知道这批是成功还是失败**。于是失败任务会先出一张红卡
   * （turn/end 的 error），等 agent 回到 idle 时又补一张绿卡（实测日志：
   * `kind=error turn=3` 紧跟着 `kind=completed turn=3`）。
   * 新的一轮从 `turn/start` 开始，那时旧的结局就作废了（所以要删掉，免得吃掉下一轮的绿卡）。
   */
  const failedRounds = new WeakMap();

  /** Raise a card for one outcome on a session. */
  const notify = (session, kind, detail, ref) => {
    ...
    fireAndForget(
      deliver(
        ...
        cwdOf({ session }), title, sessionIdOf({ session }), turn, ref
      ),
      ctx.logger, `deliver(${kind})`
    );
  };

  // --- completed: running -> idle -------------------------------------------
  const previous = new WeakMap();
  ctx.on("agent/status", guarded("agent/status", ({ agent, status }) => {
    ...
    if (failedRounds.get(agent.session) === true) {
      // 本轮已经用红卡交代过了：不要再补一张"任务已完成"（用户实测反馈的行为）。
      failedRounds.delete(agent.session);   // 结局到此作废，下一轮照常弹完成卡
      return;
    }
    failedRounds.delete(agent.session);     // 干净的一轮同样作废，避免这个标记粘住
  ...
  ctx.on("session/event", guarded("session/event", (session, event) => {
    ...
    // Track the user actions this session is waiting on, and clear the matching
    // amber row the moment the user resolves it in the GUI (approve/deny, or
    // answering an ask_user_question).
    const blockedState = nextPendingRefs(pendings.get(session), event);
    pendings.set(session, { pending: [...blockedState.pending] });
    if (shouldClearResolved(blockedState.resolved, blockedState.wasPending, event)) {
      fireAndForget(
        clearBlocked(config, ctx.logger, sessionIdOf({ session }), blockedState.resolved),
        ctx.logger, "clearBlocked"
      );
    }
    switch (event.type) {
      case "turn/end": {
        ...
        if (verdict.kind === "error") {
          failedRounds.set(session, true);   // 本轮以失败收场 → 抑制随后的完成卡
          notify(session, "error", verdict.detail);
        }
        break;
      }
      case "turn/start":
        // 这里**故意不清** failedRounds：一个 agent 跨多个排队 turn 保持 running，
        // 同一批里 turn 3 失败后 turn 4 会立刻 turn/start —— 在这里清标记，整批排空时
        // 就会又补一张绿卡（评审指出的正是这条路径）。结局统一在 agent 回到 idle 时作废。
        break;   // state already tracked above
      case "approval/asked": {
        const tool = event.data?.toolName ?? "";
        notify(session, "blocked", tool ? `等待授权：${tool}` : "等待授权", blockedState.raised);
        break;
      }
      case "tool/call": {
        if (event.data?.name === "ask_user_question") {
          notify(session, "blocked", "等待你的回答", blockedState.raised);
        }
        break;
      }
      default:
