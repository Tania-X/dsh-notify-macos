...
function startDaemon(serverPath, socketPath) {
  try {
    ...
    // A dead daemon must not keep blocking future spawns: drop the handle AND
    // reset the throttle baseline, so the next notification respawns promptly
    // (the throttle is only meant to space retries within a spawn storm).
    child.on("exit", () => {
